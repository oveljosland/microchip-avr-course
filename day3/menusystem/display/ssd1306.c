 ...
 *
 * @author      Marian Hrinko, Carl Richard Steen Fosse (edited)
 * @date        13.13.2025
 * @file        ssd1306.h
 * @version     2.0.0
 * @test        AVR128DB48
 *
 * @depend      string.h, font.h, I2C.h
 * -------------------------------------------------------------------------------------+
 ...
 *              Version for ESDI (2025) -> Adjusted for AVR128DB48 with new I2C driver.
 * -------------------------------------------------------------------------------------+
...
const uint8_t INIT_SSD1306[] PROGMEM = {
  ...
  SSD1306_SET_PAGE_ADDR, 2, 0x00, 0x07,                           // ADDED to initialize correct screen coordinates
  SSD1306_SET_COLUMN_ADDR, 2, 0x00, 0x7F,                         // ADDED to initialize correct screen coordinates
  SSD1306_DISPLAY_ON, 0                                           // 0xAF = Set Display ON
};



// @var array Cache memory Lcd 8 * 128 = 1024
...
{ 
  ...
  status = I2C_0_StartWrite(address);
  if (SSD1306_ERROR == status) {
    return status;
  }
  // Commands & Arguments
  // -------------------------------------------------------------------------------------
...
uint8_t SSD1306_Send_Command (uint8_t command)
{
  ...
  uint8_t startCommand = SSD1306_COMMAND;
  
  // send control byte
  // -------------------------------------------------------------------------------------   
  status = I2C_0_SendData(&startCommand, 1);
  if (SSD1306_ERROR == status) {
    return status;
  }
  // send command
  // -------------------------------------------------------------------------------------   
  status = I2C_0_SendData(&command, 1);
  if (SSD1306_ERROR == status) {
    return status;
  }
...
uint8_t SSD1306_UpdateScreen (uint8_t address)
{
  ...
  uint8_t streamCommand = SSD1306_DATA_STREAM;
  ...
  // -------------------------------------------------------------------------------------   
  status = I2C_0_SendData(&streamCommand, 1);
  if (SSD1306_ERROR == status) {
    return status;
  }
  ...
  // -------------------------------------------------------------------------------------
   ...
   status = I2C_0_SendData(cacheMemLcd, CACHE_SIZE_MEM);                 // send data
   if (SSD1306_ERROR == status) {
    return status;
   }
